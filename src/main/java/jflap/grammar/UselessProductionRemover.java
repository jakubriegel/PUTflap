...
import jflap.automata.UnreachableStatesDetector;
import jflap.automata.vdg.VDGTransition;
 ...
 * conjunction with <CODE>jflap.gui.jflap.grammar.transform.UselessController</CODE> it
...
public class UselessProductionRemover {
	...
	/**
	 * Returns set of all useful variables in <CODE>jflap.grammar</CODE>. A jflap.grammar
	 * is considered useful if it can derive a string.
	 * 
	 * @param grammar
	 *            the jflap.grammar
	 * @return set of all useful variables in <CODE>jflap.grammar</CODE>. A jflap.grammar
	 *         is considered useful if it can derive a string.
	 */
	...
	/**
	 * Get a jflap.grammar with only those variables that derive terminals, directly
	 * or indirectly. This is not the same as a useless production-less jflap.grammar.
	 * 
	 * @param grammar
	 *            the jflap.grammar to get the reformed jflap.grammar for
	 */
	...
	/**
	 * Returns a variable that belongs in the set of useful variables for <CODE>jflap.grammar</CODE>
	 * that is not already in <CODE>set</CODE>.
	 * 
	 * @param grammar
	 *            the jflap.grammar
	 * @param set
	 *            the set of useful variables in <CODE>jflap.grammar</CODE>.
	 * @return a variable that belongs in the set of useful variables for <CODE>jflap.grammar</CODE>
	 *         that is not already in <CODE>set</CODE>.
	 */
	 ...
	 * examines all productions in <CODE>jflap.grammar</CODE> with variable on the
	 ...
	 * derive strings) in <CODE>jflap.grammar</CODE> based on the useful variables,
	 ...
	 *         derive strings) in <CODE>jflap.grammar</CODE> based on the useful
	...
	/**
	 * Adds a state for every variable in <CODE>jflap.grammar</CODE> to <CODE>graph</CODE>,
	 ...
	 * <CODE>jflap.grammar</CODE> that has <CODE>v1</CODE> on the left hand side).
	 * 
	 * @param v1
	 ...
	 *         production in <CODE>jflap.grammar</CODE> that has <CODE>v1</CODE>
	...
	/**
	 * Returns the variable dependency graph for <CODE>jflap.grammar</CODE>.
	 * 
	 * @param grammar
	 *            the jflap.grammar
	 * @return the variable dependency graph for <CODE>jflap.grammar</CODE>.
	 */
	...
	/**
	 * Returns the set of variables in <CODE>jflap.grammar</CODE> whose productions
	 ...
	 * @param graph
	 *            the variable dependency graph
	 * @return the set of variables in <CODE>jflap.grammar</CODE> whose productions
	...
	/**
	 * Removes all productions from <CODE>jflap.grammar</CODE> that contain <CODE>variable</CODE>,
	...
	/**
	 * Returns a jflap.grammar with no variables that can not derive strings, by
	 * simply creating a new jflap.grammar and adding all productions in <CODE>usefulProductionSet</CODE>
	 * to that jflap.grammar.
	 * 
	 * @param usefulProductionSet
	 *            the set of useful productions
	 * @return a jflap.grammar with no variables that can not derive strings, by
	 *         simply creating a new jflap.grammar and adding all productions in
	 *         <CODE>usefulProductionSet</CODE> to that jflap.grammar.
	 */
	private static Grammar getGrammarWithNoVariablesThatCantDeriveStrings(
			Set<Production> usefulProductionSet) {
		Grammar g = new ContextFreeGrammar();
	...
	/**
	 * Returns a jflap.grammar, equivalent to <CODE>jflap.grammar</CODE> that contains no
	 * useless productions.
	 * 
	 * @param grammar
	 *            the jflap.grammar
	 * @return a jflap.grammar, equivalent to <CODE>jflap.grammar</CODE> that contains no
	 *         useless productions.
	 */
	public static Grammar getUselessProductionlessGrammar(Grammar grammar) {
		...
		Set<String> useless = new HashSet<String>(Arrays.asList(getUselessVariables(g, graph)));
		Production[] p = grammar.getProductions();
		for (int i = 0; i < p.length; i++) {
			Set<String> variables = new HashSet<String>(Arrays.asList(p[i].getVariables()));
