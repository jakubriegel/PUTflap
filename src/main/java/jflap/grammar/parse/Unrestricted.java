 ...
 * parse of an unrestricted jflap.grammar may, in some situations, not be recognized.
 * 
...
public class Unrestricted {
	...
	/**
	 * Returns a set of those symbols in the jflap.grammar that can derive some string
	 * smaller than it. For a normal jflap.grammar, of course, this would be just
	 * those variables with, but for an unrestricted jflap.grammar this can include
	 * the symbol <I>b</I> and <I>c</I> where <I>babca - aa</I> is a rule.
	 * <I>a</I> is not included because there are <I>a</I> terminals in the
	 * output.
	 * 
	 * @param grammar
	 *            the jflap.grammar to find the "small" symbols for
	 */
	...
	/**
	 * Returns if a jflap.grammar is truly unrestricted.
	 * 
	 * @param grammar
	 *            the jflap.grammar to test
	 * @return if a jflap.grammar is unrestricted
	 */
	...
	/**
	 * Given an unrestricted jflap.grammar, this will return an unrestricted jflap.grammar
	 * with fewer productions that accepts the same language.
	 * 
	 * @param grammar
	 *            the read jflap.grammar
	 * @return a jflap.grammar with productions some subset of the original jflap.grammar,
	 *         or <CODE>null</CODE> in the special case where no production
	 *         with just the start variable on the LHS exists (i.e. the jflap.grammar
	 *         accepts no language, though if a jflap.grammar accepts no language this
	...
	public static UnrestrictedGrammar optimize(Grammar grammar) {
		...
		Production[] prods = grammar.getProductions();
		// Which symbols in the jflap.grammar may possibly lead to just
