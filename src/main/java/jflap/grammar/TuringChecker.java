...
/**
 * Class to determine the whether loaded jflap.grammar is Turing Converted Grammar or not
...
public final class TuringChecker {
	...
	/**
	 * Checks whether loaded jflap.grammar is Turing Converted Grammar or not.
	 * @param object
	 * @return boolean whether jflap.grammar is a Turing Converted Grammar.
	 */
	public static boolean check(Serializable object) {
		...
		if (object instanceof Grammar)
		{
			...
			//check first 3 productions to make sure this jflap.grammar is from conversion
			int count=0;
			if (p.length < 3)
				return false;
			for (int i=0; i<3; i++)
