...
/**
 * The lambda production remover object can be used to convert a jflap.grammar to an
 * equivalent jflap.grammar with no lambda productions. This operation consists of 3
 ...
 * to lambdaSet. 3. Construct a new jflap.grammar with productions such that for each
 * production in the original jflap.grammar A->x1x2x3...xm, put all productions formed
 * when xj is replaced by lambda for all xj in lambdaSet. So, if there are more
 * than 1 variable in a given production from the original jflap.grammar in the
 ...
 * new jflap.grammar. You can perform this step immediately by calling
 * getLamdaProductionlessGrammar, or you can build it step by step by calling
 * getProductionsToAddForProduction for each production in the original jflap.grammar
 * and adding all returned productions to the new jflap.grammar.
 * 
...
public class LambdaProductionRemover {
	...
	/**
	 * Returns true if there is a production in <CODE>jflap.grammar</CODE> with
	 ...
	 * @param grammar
	 *            the jflap.grammar.
	 * @return true if there is a production in <CODE>jflap.grammar</CODE> with
	...
	/**
	 * Returns true if there are more variables in <CODE>jflap.grammar</CODE> that
	 ...
	 *            the jflap.grammar
	 * @param lambdaSet
	 *            the set of variables in jflap.grammar that are on the left hand side
	 *            of productions that go to lambda.
	 * @return true if there are more variables in <CODE>jflap.grammar</CODE> that
	 ...
	 * from <CODE>jflap.grammar</CODE> that has a lambda production (i.e. a
	...
	/**
	 * Adds all variables in <CODE>jflap.grammar</CODE> that have lambda transitions
	 ...
	 *            jflap.grammar that have lambda transitions).
	 */
	...
	/**
	 * Returns true if either <CODE>variable</CODE> in <CODE>jflap.grammar</CODE>
	 ...
	 * @return true if either <CODE>variable</CODE> in <CODE>jflap.grammar</CODE>
	...
	/**
	 * Returns the set of all variables in <CODE>jflap.grammar</CODE> that either
	 * have lambda productions or productions that are reducable to lambda.
	 * 
	 * @param grammar
	 *            the jflap.grammar
	 * @return the set of all variables in <CODE>jflap.grammar</CODE> that either
	...
	/**
	 * Returns a list of productions to add to a new jflap.grammar to replace <CODE>production</CODE>.
	 ...
	 *            the set of all variables that either have lambda productions
	 *            or productions that are reducable to lambda.
	 * @return a list of productions to add to a new jflap.grammar to replace <CODE>production</CODE>.
	 *         The returned list of productions are all permutations of <CODE>production</CODE>.
	 */
	...
	/**
	 * Returns all productions created by replacing each production in <CODE>jflap.grammar</CODE>
	 ...
	 *            the set of all variables that either have lambda productions
	 *            or productions that are reducable to lambda.
	 * @return all productions created by replacing each production in <CODE>jflap.grammar</CODE>
	 *         based on the variables in <CODE>lambdaSet</CODE>.
	 */
	...
	/**
	 * Returns all non lambda productions in <CODE>jflap.grammar</CODE>.
	 * 
	 * @param grammar
	 *            the jflap.grammar
	 * @return all non lambda productions in <CODE>jflap.grammar</CODE>.
	 */
	...
	/**
	 * Returns a jflap.grammar equivalent to <CODE>jflap.grammar</CODE> that has no lambda
	 * productions.
	 * 
	 * @param grammar
	 *            the jflap.grammar
