...
/**
 * This is the codec for reading JFLAP structures in the JFLAP 3 saved jflap.file
 * format.
 * 
...
public class JFLAP3Codec extends Codec {
	...
	/**
	 * Reads the jflap.file as a jflap.regular expression.
	 * 
	 * @param file
	 *            the jflap.file to read
	 * @return the jflap.regular expression associated with this document
	 */
	private RegularExpression readRE(File file) {
		...
		try {
			...
			throw new ParseException("Could not find jflap.file " + file.getName()
					+ "!");
		} catch (IOException e) {
			throw new ParseException("Error accessing jflap.file to write!");
		}
		return new RegularExpression(rstring);
	}

	/**
	 * Reads the jflap.file as a jflap.grammar.
	 * 
	 * @param file
	 *            the jflap.file to read
	 * @return the jflap.grammar associated with this document
	 */
	...
	/**
	 * Reads the jflap.file as an automaton.
	 * 
	 * @param file
	 *            the jflap.file to read
	...
	private Automaton readAutomaton(File file) {
		try {
			...
			throw new ParseException("Unexpected end of jflap.file!");
		} catch (FileNotFoundException e) {
			throw new ParseException("Could not find jflap.file " + file.getName()
					+ "!");
		} catch (IOException e) {
	 ...
	 * 
	 * @param reader
	 *            the source of lines in the jflap.file
	 */
	private jflap.automata.fsa.FiniteStateAutomaton readFA(BufferedReader reader)
			throws IOException {
		jflap.automata.fsa.FiniteStateAutomaton fa = new jflap.automata.fsa.FiniteStateAutomaton();
		// Generic states.
		...
		for (int s = 0; s < groups.length; s++) {
			for (int g = 0; g < groups[s].length; g++) {
				...
				if (group[0].equals("null"))
					group[0] = "";
				Transition t = new jflap.automata.fsa.FSATransition(from, to,
						group[0]);
				fa.addTransition(t);
			}
		}
	 ...
	 *            the source of lines in the jflap.file
	 */
	private jflap.automata.pda.PushdownAutomaton readPDA(BufferedReader reader)
			throws IOException {
		...
		jflap.automata.pda.PushdownAutomaton pda = new jflap.automata.pda.PushdownAutomaton();
		// Generic states.
		...
		for (int s = 0; s < groups.length; s++) {
			for (int g = 0; g < groups[s].length; g++) {
				...
				try {
					...
					t = new jflap.automata.pda.PDATransition(from, to, group[0],
							group[1], group[4]);
					pda.addTransition(t);
	 ...
	 *            the source of lines in the jflap.file
	 */
	private jflap.automata.turing.TuringMachine readTM(BufferedReader reader)
			throws IOException {
		...
		jflap.automata.turing.TuringMachine tm = new jflap.automata.turing.TuringMachine(
				tapes);
		// Generic states.
		...
		for (int s = 0; s < groups.length; s++) {
			for (int g = 0; g < groups[s].length; g++) {
				...
				try {
					...
					// Create the transition.
					if (tm.tapes() == 1)
						t = new jflap.automata.turing.TMTransition(from, to,
	...
	/** The JFLAP 3 jflap.file suffixes. */
	...
	/** Automaton type codes internal to the jflap.file. */
