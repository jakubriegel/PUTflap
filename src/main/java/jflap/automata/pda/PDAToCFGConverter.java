...
/**
 * The PDA to context free jflap.grammar converter can be used to convert a pushdown
 * automaton into an equivalent context free jflap.grammar. The pda and jflap.grammar will
 ...
 * and adding all of the returned productions to your context free jflap.grammar. If
 * you do this for every transition in the pda, you will have an equivalent cfg.
 * 
 * @see jflap.grammar.cfg.ContextFreeGrammar
 * 
...
public class PDAToCFGConverter {
	 ...
	 * character from the read, pop a single character from the stack and push
	 * either zero or two characters on to the stack.
	 * 
	 * @param automaton
	 *            the automaton
	 * @return true if all transitions in <CODE>automaton</CODE> read a single
	 *         character from the read, pop a single character from the stack
	 ...
	 * single character from the read, pop a single character from the stack
	 ...
	 * the read tape, pops a single character from the stack, and writes TWO
	 * characters to the stack.
	 * 
	 * @param transition
	 *            the transition
	 * @return true if <CODE>transition</CODE> reads a single character from
	 *         the read tape, pops a single character from the stack, and
	...
	public boolean isPushTwoTransition(Transition transition) {
		...
		if (toPush.length() != 2)
			return false;
		/*
		 * String read = trans.getInputToRead(); if(read.length() != 1) return
		 * false;
		 */
		String toPop = trans.getStringToPop();
		if (toPop.length() != 1)
	...
	/**
	 * Returns true if <CODE>transition</CODE> reads a single character from
	 * the read tape, pops a single character from the stack, and writes NO
	 * characters to the stack.
	 * 
	 * @param transition
	 *            the transition
	 ...
	 *         transition that pushes TWO characters on the stack.
	 */
	public ArrayList<Production> getProductionsForPushTwoTransition(Transition transition,
			Automaton automaton) {
	 ...
	 */
	public ArrayList<Production> getProductionsForPushLambdaTransition(
	 ...
	 */
	public ArrayList<Production> createProductionsForTransition(Transition transition,
			Automaton automaton) {
		ArrayList<Production> list = new ArrayList<>();
	...
	public int numberVariables() {
		return (new HashSet<String>(MAP.values())).size();
	}

	/**
	 * Returns a ContextFreeGrammar object that represents a jflap.grammar equivalent
	 ...
	 * included in the jflap.grammar.  It takes a variable and recursively checks all productions that have it on
	 ...
	 */
	private void purgeProductionsHelper(String lhs, Production[] productions, HashSet<String> valid, int[] validProductions){		
