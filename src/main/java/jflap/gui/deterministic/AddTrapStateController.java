 ...
 * This class is very similar to FSATOREController.java under jflap.gui.regular
	...
	*/	
	...
	/*	Transition t = new FSATransition(from, to, "");
		...
		else
		{
			...
			{
				JOptionPane.showMessageDialog(frame,
					"Terminal "+terminal+" is not part of readable string in DFA", "Incorrect read",
					JOptionPane.ERROR_MESSAGE);
			}
			else
			{
				JOptionPane.showMessageDialog(frame,
						"There is already a transition using Terminal "+terminal+" from this state", "Incorrect read",
						JOptionPane.ERROR_MESSAGE);
			}
			return;
		}
