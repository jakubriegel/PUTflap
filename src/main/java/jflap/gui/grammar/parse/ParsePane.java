...
import jflap.gui.TripleTextFieldSizeSlider;
...
import jflap.gui.tree.DefaultTreeDrawer;
import jflap.gui.tree.LeafNodePlacer;
import jflap.gui.tree.TreePanel;
...
abstract class ParsePane extends JPanel {
	...
	protected void initView() {
		...
		bottomHolder.add(new TableTextSizeSlider((JTable) g, JSlider.HORIZONTAL), BorderLayout.NORTH);
	...
	/**
	 * Returns the interface that holds the read area.
	 */
...
+ "(For optimization, move one of the window size adjustors around this window after resizing the text fields)"), BorderLayout.SOUTH);
	...
	/**
	 * Returns the tool bar for the main user read panel.
	 * 
	 * @return the tool bar for the main user read panel
	 */
	...
	/** The label that displays the remaining read. */
