...
class LLParseController {
	...
	/**
	 * Sets the current stack and remaining read displays in the view to
	...
	public void step() {
		...
		case NORMAL:
			...
			if (pane.grammar.isTerminal(top)) {
				if (top.equals(read)) {
					...
					stepMode = ERROR;
					pane.statusDisplay.setText("Stack and read don't match.");
				}
				updateStatus();
				return;
			}
		...
		case SUCCESS:
			...
			if (!read.equals("$")) {
				pane.statusDisplay
						.setText("The stack is empty, but the read is not.");
				stepMode = ERROR;
			} else {
				pane.stepAction.setEnabled(false);
