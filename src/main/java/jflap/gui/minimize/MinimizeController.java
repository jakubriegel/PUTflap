...
import jflap.automata.fsa.MinimizeTreeNode;
import jflap.automata.fsa.Minimizer;
import jflap.gui.tree.SelectTreeDrawer;
import jflap.gui.tree.Trees;
 ...
 * <CODE>Minimizer</CODE> object and the various jflap.gui classes that handles the
 ...
 * {@link jflap.gui.minimize.BuilderController}.
 * 
...
class MinimizeController {
	...
	public void toggleState(MinimizeTreeNode node, State state) {
		...
		try {
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (child == node)
					continue;
				Collection<State> c = Arrays.asList(child.getStates());
				if (c.contains(state)) {
					JOptionPane.showMessageDialog(view,
		...
		List<State> list = new LinkedList<>(Arrays.asList(states));
	...
	private void split(MinimizeTreeNode node) {
		...
		ArrayList<State[]> groups = minimizer.splitOnTerminal(node.getStates(), node
	...
	public boolean check(MinimizeTreeNode node) {
		...
		Iterator<State[]> it = groups.iterator();
		while (it.hasNext())
			realPartitions.add(new HashSet<State>(Arrays.asList((State[]) it.next())));
