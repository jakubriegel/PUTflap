...
/**
 * The <CODE>InputTableModel</CODE> is a table specifically used for the read
 ...
 * wants to treat it as a transducer), and the final output.
 * 
...
public class InputTableModel extends GrowableTableModel {
	 ...
	 * 
	 * @param columns
	 *            the number of columns for this read table model
	 */
	...
	/**
	 * This returns the name of the columns. In the read table model, each of
	 * the first {@link #getColumnCount} columns is an read column and is
	 ...
	 * one read column, in which case the single read column is just called
	 * "Input". There are as many output columns as read columns, and are
	 * numbered in a similar fashion except with the prefix "Output" instead of
	 * "Input". The last column is always reserved for the output.
	 * 
	 * @param column
	...
	/**
	 * This returns an array of the inputs for the table. The read is organized
	 * by arrays of arrays of strings. The first index of the array is the row
	 * of the read. The second index of the array is the particular read,
	 ...
	 * @return the number of read strings needed for this automaton; e.g., n
	 ...
	 * desirable that automatons, upon asking to run read, should be presented
	 * with the same data in the same table since multiple inputs tables are oft
	 * used to test the same sets of read on different automaton's again and
	...
	/**
	 * Sets the output string for a particular row. If you wanted to clear a row
	 * of all results, you would call this function <CODE>setResult(row, "",
	 * null)</CODE>.
	 * 
	 * @param row
	 *            the row to set the output of
	 * @param result
	 *            the output to put in the output column
	 * @param config
	 ...
	 * have been read via the {@link #setResult} method.
	 * 
	 * @param row
	...
	/**
	 * The map of number of inputs (stored as integers) to read table models.
	 */
