...
package jflap.gui.menu;


import jflap.automata.Automaton;
import jflap.automata.fsa.FiniteStateAutomaton;
import jflap.automata.graph.LayoutAlgorithmFactory;
import jflap.automata.graph.layout.VertexMover;
import jflap.grammar.TuringChecker;
import jflap.gui.action.*;
		...
		if (Universe.CHOOSER != null) {
            ...
            if (environment instanceof AutomatonEnvironment){ //this is strictly for non-Grammar
                ...
                if (new File("svg.jar").exists() || (jar != null && jar.getJarEntry("org/foo.txt") != null)){
                    ...
                    try{
                        RestrictedAction ra = (RestrictedAction) Class.forName("jflap.gui.action.ExportAction").getConstructor(new Class[]{Environment.class}).newInstance(environment);
...
//        }
	...
	/**
	 * Instantiates the menu that holds read related menu events.
	 * 
	 * @param frame
	 *            the environment frame that holds the environment and object
	 * @return an read menu
	...
	private static JMenu getInputMenu(EnvironmentFrame frame) {
		 ...
		 * GrammarOutputAction ((jflap.gui.environment.GrammarEnvironment)
		 * environment));
		 */
		...
		if (LLParseTableAction.isApplicable(object))
			addItem(menu, new LLParseTableAction(
					(jflap.gui.environment.GrammarEnvironment) environment));
...
//					(jflap.gui.environment.GrammarEnvironment) environment));
		...
		if (LSystemDisplay.isApplicable(object))
			addItem(menu, new LSystemDisplay(
					(jflap.gui.environment.LSystemEnvironment) environment));

		return menu;
	}

	/**
	 * Get read menu for turing converted grammar
	 * @param frame
	...
	private static JMenu getTestMenu(EnvironmentFrame frame) {
		...
		if (DFAEqualityAction.isApplicable(object))
			addItem(menu, new DFAEqualityAction(
					(FiniteStateAutomaton) object, environment));
		...
		if (NondeterminismAction.isApplicable(object))
			addItem(menu, new NondeterminismAction((Automaton) object,
					environment));
		/*
		...
		/*
		  ...
		  GrammarTestAction ((jflap.gui.environment.GrammarEnvironment)
		  environment));
		 */

		if (GrammarTypeTestAction.isApplicable(object))
			addItem(menu, new GrammarTypeTestAction ((jflap.gui.environment.GrammarEnvironment) environment));
		return menu;
	}
	...
	private static JMenu getConvertMenu(EnvironmentFrame frame) {
		...
		if (ConvertFSAToGrammarAction.isApplicable(object))
			addItem(menu, new ConvertFSAToGrammarAction(
					(jflap.gui.environment.AutomatonEnvironment) environment));
		...
		if (REToFSAAction.isApplicable(object))
			addItem(menu, new REToFSAAction(
					(jflap.gui.environment.RegularEnvironment) environment));
